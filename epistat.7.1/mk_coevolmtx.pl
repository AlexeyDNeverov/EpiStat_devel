...
#			Default="."
#		NonNegativeElementFilter="0|1" - Set to zero negative elements in the matrix
#			Default="0"
...
#			Default="0"
#		DistatnceType="SYM"|"ASYM" - The type of site pairs: symetric or asymetric
#			Default="ASYM"
...
while(<INFILE>){
	...
	if(/\[\s*(.+)\s*\]/){
		...
		if($key eq "NumberOfDatasets"){
			...
			die "The parameter 'DistatnceType' must be 'SYM' OR 'ASYM'!" unless $value=~m/^sym|asym/i;
...
if($datasets_num>0&&$datasets_num<=4){
	...
	if($general_settings->distance_type eq "asym"){
		for(my $i=0;$i<$datasets_num;$i++){
