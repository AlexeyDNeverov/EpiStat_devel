...
if($nperm){
	...
	unless(check_outfiles($name,@out_file_types)){
		...
		if($nperm>$max_items_indir){
			...
			$h0=EpistatNullModel->new("-size" => $nperm,"-samples_dir" => $samples_dir,"-stats_ext" => $stats_ext,"-obs_pairs_fn" => $pairs_fn,
				"-skip_obs_id" => $sample_id, "-is_intragene" => $f_intragene, "-max_samples_indir" => $max_items_indir);
		}else{
			$h0=EpistatNullModel->new("-size" => $nperm,"-samples_dir" => $samples_dir,"-stats_ext" => $stats_ext,"-obs_pairs_fn" => $pairs_fn,
