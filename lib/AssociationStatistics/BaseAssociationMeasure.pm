...
sub _init{
	...
	{
		my @tmp_data=($self->{OBS});
		my @col_idx=(1);
		my $nlines=IO::SitePairIdxData::airead_file($str,5,$self->{OBS_LINES},\@tmp_data,\@col_idx);
		if(($general_settings->distance_type eq "allele")&&($dataset_desc->f_intragene_pairs)){
			#The matrix is upper triangular. Add complementary elements.
			$nlines*=2;
			my @tmp_obs;
			my $n=@{$self->{OBS_LINES}};
			for(my $i=0;$i<$n;$i++){
				my $ln=$self->{OBS_LINES}->[$i];
				my $obs=$self->{OBS}->[$i];
				push @tmp_obs,[($ln,$obs)];
				my ($bgs,$tgs)=$self->line2sites_idxs($ln);
				$ln=$self->site_idx_pair2line($tgs,$bgs);
				push @tmp_obs,[($ln,$obs)];
			}
			@{$self->{OBS_LINES}}=();
			@{$self->{OBS}}=();
			foreach my $pi(sort {$a->[0]<=>$b->[0]} @tmp_obs){
				push @{$self->{OBS_LINES}},$pi->[0];
				push @{$self->{OBS}}, $pi->[1];
			}
		}
		die "\nError in AssociationStatistics::BaseAssociationMeasure::_init(): Unexpected number of elements in $str:\n\t$nlines\texpected: ".$self->{NLINES}."!" unless $self->{NLINES}==$nlines;
	}
	$str=$dataset_desc->mean_fn;
	...
	if(defined($dataset_desc->bgr_selected_sites_fn)||defined($dataset_desc->bgr_selected_sites_fn)){
		my $f_ord_pairs=1;
		$f_ord_pairs=0 if($dataset_desc->f_intragene_pairs);
		my $f=AssociationStatistics::SelectedSitesFilter->new($self,0,$dataset_desc->bgr_selected_sites_fn,$dataset_desc->fgr_selected_sites_fn,$f_ord_pairs,$dataset_desc->f_both_sites_selected);
		push @{$self->{FILTERS}},$f;
	}
}
