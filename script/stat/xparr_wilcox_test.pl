...

{
	...
	my $ntg=0;
	my $nbg=0;
	for(my $I=0;!(eof(PAIRS)||eof(ORDSTAT));$I++){
		my $str=<PAIRS>;
...
sub get_sites_keys{
	...
	@keys=sort {
		return $hr_sites->{$b}->ordering_stats <=> $hr_sites->{$a}->ordering_stats if $sort_order;
		$hr_sites->{$a}->ordering_stats <=> $hr_sites->{$b}->ordering_stats} @keys;
	return @keys;
}
...
sub get_pairs_keys{
	...
	return sort {
		return $ha_site_pairs->[$b]->[2]->ordering_stats <=> $ha_site_pairs->[$a]->[2]->ordering_stats if $sort_order;
		$ha_site_pairs->[$a]->[2]->ordering_stats <=> $ha_site_pairs->[$b]->[2]->ordering_stats} keys(%hkeys);
}
