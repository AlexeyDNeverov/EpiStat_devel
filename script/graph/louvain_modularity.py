...
parser.add_option("-n", "--neg", dest="neg",
                  help="specifies negative graph layer")
parser.add_option("-p", "--pos", dest="pos",
                  help="specifies positive graph layer")
...
if not((options.funct == "modularity") or 
	...
	parser.error("Unsupported functional accounted!")
...
while n_iters>0:
	if options.neg:
		...
		diff = optimiser.optimise_partition(part_pos)
	sign=louvain.SignificanceVertexPartition.FromPartition(part_pos).quality()
...
print("Q=",quality,"Significance=",significance)
print tabulate(zip(G_pos.vs["id"],part_pos.membership),headers=['site','group'])