...
if not((options.funct == "modularity") or 
	(options.funct == "surprise") or (options.funct == "CPM")):
...
if options.funct == "modularity":
	...
	part_pos = la.SurpriseVertexPartition(G_pos, weights='weight')
elif options.funct == "CPM":
	part_pos = la.CPMVertexPartition(G_pos, weights='weight')
diff=1
if options.neg:
	...
	if options.funct == "modularity":
		...
		part_neg = la.SurpriseVertexPartition(G_neg, weights='weight')
	elif options.funct == "CPM":
		part_neg = la.CPMVertexPartition(G_neg, weights='weight')
...
significance=la.SignificanceVertexPartition.FromPartition(part_pos).quality()
...
if options.neg:
	quality=quality-part_neg.quality()
membership=part_pos.membership
print("Q=",quality,"Significance=",la.SignificanceVertexPartition.FromPartition(part_pos).quality())
